...
// 获取当前用户的ip_ck，并进行解码，获取时间戳，然后跟10取模
/*
 * Javascript base64_encode() base64加密函数
   用于生成字符串对应的base64加密字符串
 * 吴先成  www.51-n.com ohcc@163.com QQ:229256237  http://www.51-n.com/t-4183-1-1.html
...
//ipck = '5caG4Pryj7QuMjE3NzA1LjE0NTU3ODkzNDI=';
var res1 = base64_decode(ipck);
// 默认zol规则
var delivery = '1';

if(res1.search(/\d{4,}/) != -1){
	// 获取字串中的时间戳     xxxx.217705.1455789342
	var resStamp = res1.match(/.(\d{10,})/);
	resStamp = Number(resStamp[1]);
	delivery = resStamp % 10;
	delivery = delivery.toString();
}else{
	//alert('没有匹配到');
}

//alert($.inArray(delivery,['0','1','2'])!=-1);

// 012 表示百度；345表示zol；6789表示百分点
if($.inArray(delivery,['0','1','2'])!=-1){
	ourRule = 'baidu';
}else if($.inArray(delivery,['3','4','5'])!=-1){
	ourRule = 'zol';
}else if($.inArray(delivery,['6','7','8','9'])!=-1){
	ourRule = 'baifendian';
}

//alert(typeof(base64_decode));
console.log('你的规则是：'+ourRule);
...
(function($){
	...
	else if(typeof ourRule != 'undefined' && ourRule == 'baidu'){
		// 针对多版本整合成一个首页推荐 20160219
		var sc = document.createElement("script");
		sc.type = "text\/javascript";
		sc.src = "http://crs.baidu.com/tapi.js?planId=23453&siteId=ae5edc2bc4fc71370807f6187f0a2dd0";
		document.getElementsByTagName('HEAD').item(0).appendChild(sc);
		window.onload=function(){
			if($('.you-like .you-like-list-box li a').length>1){
				$('#hm_t_23453').fadeIn();
			}
		}
