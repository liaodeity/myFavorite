...
// $ip_key = '5M6B4fLxj7QuMzkwNjg2LjE0NDMzMjUzNTg=';
/* 压测时用到的代码  S */
// include('./ip_cp_array.php');
// $icNum = count($ipckArr);
// $icIndex = mt_rand(0, $icNum-1);
// $ip_key = $ipckArr[$icIndex];
/* 压测时用到的代码 E */
...
# 获取用户读取的doc_id
$docArr = array();
// var_dump($newArr['article_id']);exit('80_3');
...
if($newArr['article_id']){
	...
	# 根据第一个用户浏览的文章id，取最高优先级的规则的数据.
	$resDataAll = get_arti_by_word($docArr);
	$res1_0 = $resDataAll['article'];
	//var_dump($res1_0);exit('91');
	$num0 = count($res1_0);# 记录进入日志
	if($num0 <= 0) {
		$docStr = implode(',',$docArr);
		// toLog($docStr);
	}
	# 第一优先级取出的数据不足36的时候   $num0 < 36  true
	if($num0 < 36){
		# 获取第一优先级的数据的docId
		$res1_0_1 = array();
		# 储存第一优先级的文章id，用于排重
		...
		//var_dump($proId);exit('104_1');
		# 没有proId则使用“看了又看”   $proId
		if($proIdArr){
			# 返回形如array(0=>5255718,1=>5255719...)
			$docArr1_1 = get_docid_by_proid($proIdArr,36-$num0,$unArr);
			//var_dump($docArr1_1);exit('121_1');
			# 通过proId获得了文章id
	...
	//$num < 36
	if($num < 36){
		$lookMoreDocId = get_docid_by_lookmore($docArr,36-$num,$unArr);
		$lookDocIdArr = array_keys($lookMoreDocId);

		$docArr1_2 = $docArr1_2 + $lookDocIdArr;
		...
		$resDocArr2 = count($docArr1_2) < 36 ? get_data_by_hot_v3($randNum,36-$num,$docArr1_2,true) : array();
	...
	$fResult =  get_data_by_hot_v3($randNum,36,array(),true);
}else{
...
$keyPositionArr = array(1,2,3,5,6,7,9,10,11,13,14,15,17,18,19,21,22,23,25,26,27,29,30,31,33,34,35); 
# 随机获取两个文字链的位置
...
/*********************************以下是一些需要用到的函数**********************************************************/
# B计划获取数据,从domain获取数据
function get_data_from_domain($domainData,$num=0,$undocArr=array(),$fields='document_id,title,url,pic_src'){
	global $db_doc;
	if($domainData){
		$needDomain = array();
		...
		$domainStr = implode(',', $needDomain);
		$channelWhere = 'AND channel in('.$domainStr.')';
	}else{
		$channelWhere = '';
	}
	# 设定需要加载的字段
	$fields='document_id,title,url,pic_src';
	$unIdStr = implode(',',$undocArr);
	$sql = 'select '.$fields.' from article_monitor_all where document_id not in('.trim($unIdStr,',').') AND publish_time>"'.date('Y-m-d',strtotime('-3 days')).'" AND is_del=0 '.$channelWhere.' order by score_time_position desc limit '.$num;
	
	$result = $db_doc -> get_results($sql,'A');
	if($result){
		foreach($result as $key=>$value){
			$result[$key]['url'] = $value['url'].'?vlike=miss';
		}
	}
	$getNum = count($result);
	if(is_array($result) &&  $getNum >= $num){
		return $result;
	}else{
		# C计划获取数据
		# 将B计划获取的数据排重处理
		if(is_array($result) && $result){
			foreach($result as $key=>$value){
				$undocArr[] = $value['document_id'];
			}
		}
		$unIdStr = implode(',',$undocArr);
		# 需要补偿的条数
		$needNum = $num - $getNum;
		$sql = 'select '.$fields.' from article_monitor_all where document_id not in('.trim($unIdStr,',').') AND publish_time>"'.date('Y-m-d',strtotime('-3 days')).'" AND is_del=0  order by score_time_position desc limit '.$needNum;
		...
		$result2 = array_merge($result,$result2);
	...
	if($result){
		foreach($result as $key=>$value){
			...
			$getDataArr = ZOL_Api::run("Kv.MongoCenter.get" , array(
					'module'         => 'cms',           		#业务名
					'tbl'            => 'zol_index',     		#表名
					'key'            => 'guess_you_like_index'.$value['document_id'], 	#key
			));
			# 暂时不适用缓存 is_array($getDataArr) && $getDataArr
			if(false){
				# 对使用了mongo缓存的数据进行标记
 ...
 */
function get_arti_by_word($docIdArr,$num=36){
	if(!$docIdArr) return false;
	global $db_guess,$randNum;
...
on t.word=t2.word where t2.word is not null AND t.bbsUrl="" GROUP BY t.article_id ORDER BY uv desc limit 60)UNION (SELECT t.article_id,t.uv,t.word,t.title,t.page_type_id,t2.word as w,count(*) as cnt,t.bbsUrl,t.flag from (SELECT word from tongji_article_title_words where '.$docIdStr.') as t2 LEFT JOIN  tongji_article_title_words t
